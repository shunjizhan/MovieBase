...
-- For Sales
insert into Sales value (1111,1174130,17611950); -- violate violates the primary key constraints / Duplicate entry '1111' for key 'PRIMARY'

insert into Sales value (11111,1174130,17611950); -- violates the foreign key constraints / Cannot add or update a child row: a foreign key constraint fails (`CS143`.`Sales`, CONSTRAINT `Sales_ibfk_1` FOREIGN KEY (`mid`) REFERENCES `Movie` (`id`))

insert into Sales value (1568,-1,-100); -- violates the CHECK constrains
...
-- For MovieRating
insert into MovieRating value (3561,120,120); -- violates the CHECK constrains / Duplicate entry '3561' for key 'PRIMARY'

insert into MovieRating value (3561,89,33); -- violates the primary key constraints / Duplicate entry '3561' for key 'PRIMARY'
